library(nimble)
library(coda)
source("sim.JS.SMR.Dcov.Generalized.Interspersed.TrapRE.R")
source("init.SMR.Dcov.Open.Generalized.Interspersed.TrapRE.R")
source("Nimble Model JS-SMR-Dcov Generalized Interspersed TrapRE.R")
source("Nimble Functions JS-SMR-Dcov Generalized Interspersed TrapRE.R") #contains custom distributions and updates
source("sSampler Dcov Open Marginal Generalized Interspersed TrapRE.R") # activity center sampler that proposes from prior when z.super=0.
source("mask.check.R")
#must run this line 
...
data <- sim.JS.SMR.Dcov.Generalized.Interspersed.TrapRE(D.beta0=D.beta0,D.beta1=D.beta1,D.cov=D.cov,tau=tau,
...
#str(data$y.mID) #marked with ID sighting history: n.marked.all x n.primary x J.sight.max x K.sight.max
#str(data$y.mnoID) #marked with no ID sighting history: n.primary x J.sight.max x K.sight.max
#str(data$y.um) #unmarked sighting history: n.primary x J.sight.max x K.sight.max
#str(data$y.unk) #unknown marked status sighting history: n.primary x J.sight.max x K.sight.max
#str(data$mark.states) #mark status history: n.marked.all x n.primary x K.sight.max (additional requirement for interspersion)
#str(data$mark.states2D) #mark status history: n.marked.all x n.primary
...
inits <- list(p0=rep(0.1,n.primary),lam0=rep(0.25,n.primary),
              sigma=rep(0.5,n.primary),theta.d=rep(10,n.primary)) #initializing with 1 parameter per session, just set all to same value
...
constants <- list(n.primary=n.primary,M=M,J.mark=J.mark,J.sight=J.sight,K.sight=K.sight,xlim=xlim,ylim=ylim,
                  K1D.mark=nimbuild$K1D.mark,K2D.sight=nimbuild$K2D.sight,tau=tau,
...
Nimdata <- list(y.mark=nimbuild$y.mark, #marking process
                ...
                mark.states=nimbuild$mark.states, #mark state history (who is marked in each primary occasion on each occasion)
...
parameters <- c('N','gamma','N.recruit','N.survive','N.super','lambda.y1',
                'phi.fixed','p0','lam0','sigma.fixed','theta.d.fixed','theta.marked','theta.unmarked',
                'D0','D.beta1')
nt <- 1 #thinning rate
...
config.nodes <- c('phi.fixed','gamma','p0','lam0','sigma.fixed','theta.d.fixed','theta.marked','theta.unmarked[2:3]')
...
calcNodes <- c(N.nodes,N.recruit.nodes,y.mark.nodes,y.um.nodes,y.unk.nodes,trap.RE.nodes,z.nodes,tel.z.states.nodes) #the ones that need likelihoods updated in mvSaved
...
library(vioplot)
vioplot(mvSamples[-c(1:500),3:(n.primary+2)],ylim=c(0,200),
