...
# source("Nimble Functions JS-SMR-Dcov Generalized Interspersed.R") #contains custom distributions and updates
# source("sSampler Dcov Open Marginal Generalized Interspersed.R") # activity center sampler that proposes from prior when z.super=0.
source("mask.check.R")
#must run this line 
...
K.mark <- c(8,0,0,8,0,0) #yearly marking occasions
...
#Set K.order[[g]] <- NA if no sighting effort in year g
...
Nimdata <- list(y.mark=nimbuild$y.mark, #marking process
                ...
                mark.states=nimbuild$mark.states, #mark state history (who is marked in each year on each occasion)
...
config.nodes <- c('phi.fixed','gamma.fixed','p0','lam0','sigma.fixed','theta.d.fixed','theta.marked','theta.unmarked[2:3]')
