library(nimble)
library(coda)
source("sim.JS.SMR.Dcov.Generalized.Interspersed.R")
source("init.SMR.Dcov.Open.Generalized.Interspersed.R")
source("Nimble Model JS-SMR-Dcov Generalized Interspersed.R")
...
data <- sim.JS.SMR.Dcov.Generalized.Interspersed(D.beta0=D.beta0,D.beta1=D.beta1,D.cov=D.cov,
            InSS=InSS,phi=phi,gamma=gamma,n.year=n.year,K.order=K.order,
...
#str(data$mark.states) #mark status history: n.marked.all x n.year x K.sight.max (additional requirement for interspersion)
#str(data$mark.states2D) #mark status history: n.marked.all x n.year
...
nimbuild <- init.SMR.Dcov.Open.Generalized.Interspersed(data,inits,M=M)
...
constants <- list(n.year=n.year,M=M,J.mark=J.mark,J.sight=J.sight,K.sight=K.sight,xlim=xlim,ylim=ylim,
...
lam.um.nodes <- Rmodel$expandNodeNames(paste0("lam.um[1:",n.year,",1:",max(J.sight),",1:",max(K.sight),"]"))
lam.unk.nodes <- Rmodel$expandNodeNames(paste0("lam.unk[1:",n.year,",1:",max(J.sight),",1:",max(K.sight),"]"))
y.um.nodes <- Rmodel$expandNodeNames(paste0("y.um[1:",n.year,",1:",max(J.sight),",1:",max(K.sight),"]"))
y.unk.nodes <- Rmodel$expandNodeNames(paste0("y.unk[1:",n.year,",1:",max(J.sight),",1:",max(K.sight),"]"))
