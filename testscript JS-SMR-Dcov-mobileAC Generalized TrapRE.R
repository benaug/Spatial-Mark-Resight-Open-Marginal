...
data <- sim.JS.SMR.Dcov.mobileAC.Generalized.TrapRE(D.beta0=D.beta0,D.beta1=D.beta1,D.cov=D.cov,tau=tau,
...
config.nodes <- c('phi.fixed','gamma','p0','lam0','sigma.fixed','theta.d.fixed',
...
for(i in 1:M){
  for(g in 1:n.primary){
    ...
    if(J.sight[g]>0){
      ...
      y.um.nodes <- Rmodel$expandNodeNames(paste0("y.um[",g,",1:",J.sight[g],"]"))
      ...
      if(i<=nimbuild$n.marked.all){
        ...
        lam.mnoID.nodes <- Rmodel$expandNodeNames(paste0("lam.mnoID[",g,",1:",J.sight[g],"]"))
      }else{
