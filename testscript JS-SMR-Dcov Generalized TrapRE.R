library(nimble)
library(coda)
source("sim.JS.SMR.Dcov.Generalized.TrapRE.R")
...
data <- sim.JS.SMR.Dcov.Generalized.TrapRE(D.beta0=D.beta0,D.beta1=D.beta1,D.cov=D.cov,tau=tau,
...
nimbuild <- init.SMR.Dcov.Open.Generalized.TrapRE(data,inits,M=M)
...
parameters <- c('N','gamma','N.recruit','N.survive','N.super','lambda.y1','theta.d.fixed',
...
config.nodes <- c('phi.fixed','gamma','p0','lam0','sigma.fixed','theta.marked','theta.unmarked[2:3]','theta.d.fixed')
...
calcNodes <- c(N.nodes,N.recruit.nodes,y.mark.nodes,y.um.nodes,y.unk.nodes,
...
rem.idx <- c(grep("N",colnames(mvSamples)),
             grep("theta.marked",colnames(mvSamples)),
             grep("theta.unmarked",colnames(mvSamples)))
