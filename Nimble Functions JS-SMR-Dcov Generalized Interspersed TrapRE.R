...
dDetectorNBCorrection <- nimbleFunction(
  run=function(x=double(0),y.trap.total=double(2),bigLam.marked=double(2),bigLam.unmarked=double(2),
               K2D=double(2),theta=double(0),J=integer(0),K=integer(0),log=integer(0)){
    returnType(double(0))
    if(theta<=0){
      if(log){
        return(-Inf)
    ...
    for(j in 1:J){
      ...
      count <- 0
      for(k in 1:K){
        mu <- mu+K2D[j,k]*(bigLam.marked[j,k]+bigLam.unmarked[j,k])
        count <- count+y.trap.total[j,k]
      }
      logProb <- logProb+lgamma(theta+count)-lgamma(theta)+theta*log(theta)+mu-
        (theta+count)*log(theta+mu)
    }
    if(log){
      return(logProb)
    }else{
...
rDetectorNBCorrection <- nimbleFunction(
  run=function(n=integer(0),y.trap.total=double(2),bigLam.marked=double(2),bigLam.unmarked=double(2),
               K2D=double(2),theta=double(0),J=integer(0),K=integer(0)){
    returnType(double(0))
    return(0)
  }
)
    ...
    for(i in 1:n.cap.all){
      if(y2D[i,1]==0){ #skip if known to be alive in 1st primary occasion
        ...
        for(g in 1:first.det){ #must be recruited in primary occasion with first detection or before
          ...
          if(length(sight.before)>0){
            #shared marginalized likelihood nodes: recalculate the complete vectors.
            #Unaffected occasions contribute the same constant to every Gibbs candidate.
            lp.y.um <- model$calculate(y.um.nodes)
            lp.y.unk <- model$calculate(y.unk.nodes)
          }
          lp.trap.RE <- model$calculate(trap.RE.nodes)
    ...
    for(i in (n.cap.all+1):M){
      if(model$z.super[i]==1){
        ...
        if(z.start.prop!=z.start.curr|z.stop.prop!=z.stop.curr){
          ...
          #only marking likelihoods in primary occasions where z changed
          lp.initial.y.mark <- 0
          ...
          #only sighting likelihoods in primary occasions where z changed
          lp.initial.y.um <- 0
          lp.initial.y.unk <- 0
          ...
          #only subtract current lambdas in sighting primary occasions where z changed
          for(g2 in 1:n.sight.g){
            gg <- sight.g[g2]
          ...
          #only update pd in marking primary occasions where z changed
          for(g2 in 1:n.mark.g){
            gg <- mark.g[g2]
            if(z.prop[gg]!=z.curr[gg]){
          ...
          #only update lam in sighting primary occasions where z changed
          for(g2 in 1:n.sight.g){
            gg <- sight.g[g2]
            if(z.prop[gg]!=z.curr[gg]){
          ...
          #only add proposed lambdas in sighting primary occasions where z changed
          for(g2 in 1:n.sight.g){
            gg <- sight.g[g2]
          ...
          if(accept){
            ...
            mvSaved["N.recruit",1] <<- model[["N.recruit"]]
            mvSaved["ER",1] <<- model[["ER"]]
            #only changed pd nodes need to be saved
            for(g2 in 1:n.mark.g){
              gg <- mark.g[g2]
              if(z.prop[gg]!=z.curr[gg]){
                for(j in 1:J.mark[gg]){
            ...
            #only changed sighting nodes need to be saved
            for(g2 in 1:n.sight.g){
              gg <- sight.g[g2]
              if(z.prop[gg]!=z.curr[gg]){
                mvSaved["bigLam.unmarked",1][gg,1:J.sight[gg],1:K.sight[gg]] <<- model[["bigLam.unmarked"]][gg,1:J.sight[gg],1:K.sight[gg]]
            ...
            model[["N.recruit"]] <<- mvSaved["N.recruit",1]
            model[["ER"]] <<- mvSaved["ER",1]
            #restore pd only in marking primary occasions where z changed
            for(g2 in 1:n.mark.g){
              gg <- mark.g[g2]
              if(z.prop[gg]!=z.curr[gg]){
                for(j in 1:J.mark[gg]){
            ...
            #restore sighting nodes only in primary occasions where z changed
            for(g2 in 1:n.sight.g){
              gg <- sight.g[g2]
              if(z.prop[gg]!=z.curr[gg]){
                model[["bigLam.unmarked"]][gg,1:J.sight[gg],1:K.sight[gg]] <<- mvSaved["bigLam.unmarked",1][gg,1:J.sight[gg],1:K.sight[gg]]
            ...
            model$calculate(N.nodes[1])
            #only restore marking logProbs where z changed
            for(g2 in 1:n.mark.g){
              gg <- mark.g[g2]
              if(z.prop[gg]!=z.curr[gg]){
            ...
            #only restore aggregate sighting and TrapRE logProbs where z changed
            for(g2 in 1:n.sight.g){
              gg <- sight.g[g2]
              if(z.prop[gg]!=z.curr[gg]){
                node.start <- sight.node.start[g2]
